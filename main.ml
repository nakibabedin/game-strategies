...
module Exercises = struct
  ...
  let _heuristic_function_2 ~(game : Game.t) ~(piece : Game.Piece.t) =
    let score_part_1 =
      50 * -List.length (winning_moves game ~me:(Game.Piece.flip piece))
    in
    let occupied_positions = Map.keys game.board in
  ...
  let best_next_move_minimax ~game ~piece : Game.Position.t =
    ...
    | false ->
      let possible_next_moves =
        ...
        | 0 -> winning_moves ~me:piece game
        | _ ->
          (match
             List.length (winning_moves ~me:(Game.Piece.flip piece) game)
           with
           | 0 -> winning_moves ~me:(Game.Piece.flip piece) game
           | _ ->
             (match
                List.length
